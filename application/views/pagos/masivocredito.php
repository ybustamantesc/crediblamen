								...
								<div class="form-group row">
										...
										<div class="form-group">
											...
											<select class="form-control select2" name="idcliente" id="idcliente" required <?php echo (isset($pago) && $pago->estado == 1 ? 'disabled' : ''); ?>>
												...
												<?php foreach ($clientes as $cliente) : ?>
													<?php if (isset($pago)) : ?>
														<option value="<?php echo $cliente->idcliente; ?>" <?php echo ($cliente->idcliente == $pago->idcliente ? 'selected' : '') ?>><?php echo $cliente->dni . ' - ' . $cliente->apellidos . ', ' . $cliente->nombres; ?></option>
													<?php else : ?>
														<option value="<?php echo $cliente->idcliente; ?>"><?php echo $cliente->dni . ' - ' . $cliente->apellidos . ', ' . $cliente->nombres; ?></option>
													<?php endif; ?>
												<?php endforeach; ?>
											</select>
									...
									<div class="col-md-3">
										<div class="form-group">
											...
											<input type="text" class="form-control" readonly name="total_pagar" id="total_pagar" required value="<?php echo (isset($pago) ? $pago->fecha_credito : set_value('fecha_credito')); ?>">
