                                                                ...
                                                                <div class="modal-content">
                                                                    ...
                                                                    <div class="modal-body">
                                                                        ...
                                                                        <div class="desembolso-preview-grid">
                                                                            <div class="desembolso-preview-cheque">
    ...
    $(document).on('click', '.btnVerCheque', function(){
        ...
        $.get('<?php echo site_url('tesoreria/get_movimiento_ajax'); ?>', {id: id}, function(resp){
            ...
            if (j && j.status && j.movimiento) {
                renderDetalleDesembolso(j.movimiento);
            } else {
                $('#desembolsoPreviewPanel').hide();
                $('#btnAnularDesembolsoDesdeModal').hide();
            }
        ...
        $.get('<?php echo site_url('tesoreria/get_movimientos_ajax'); ?>', {desde: desde, hasta: hasta, tipo: tipoDoc}, function(resp){
            ...
            if(j && j.status && Array.isArray(j.movimientos)){
                j.movimientos.forEach(function(mov){
                    var rowClass = mov.estado === 'anulado' ? 'table-danger text-muted' : (mov.estado === 'previsualizacion' ? 'table-warning' : '');
                    var motivoAnulacion = mov.estado === 'anulado' && mov.motivo_anulacion ? '<span title="'+mov.motivo_anulacion.replace(/"/g,'&quot;')+'" class="badge badge-warning">Anulado</span>' : '';
                    var acciones = '';
                    var esDesembolso = String(mov.tipo || '') === 'desembolso_preview' || String(mov.referencia2 || '').indexOf('p=') === 0;
                    if (mov.estado === 'previsualizacion') {
                        acciones = '<button class="btn btn-sm btn-warning btnVerCheque" data-id="'+mov.id+'" data-preview="1">Mostrar</button> ';
                    } else if(mov.estado !== 'anulado') {
                        if ((mov.forma_pago || '').toUpperCase() === 'CHEQUE' || (mov.tipo_movimiento || '').toLowerCase() === 'cheque') {
                            acciones = '<button class="btn btn-sm btn-info btnVerCheque" data-id="'+mov.id+'">Ver</button> ';
                        }
                        if (esDesembolso) {
                            acciones += '<button class="btn btn-sm btn-secondary" disabled style="pointer-events:none;opacity:0.65;">Anular bloqueado</button> ';
                        } else {
                            acciones += '<button class="btn btn-sm btn-danger btnAnularMov" data-id="'+mov.id+'">Anular</button> ';
                        }
                        if (mov.tiene_asiento == 1) {
                            acciones += '<button class="btn btn-sm btn-secondary btnContabilizarMov" data-id="'+mov.id+'" disabled style="pointer-events:none;opacity:0.6;">Contabilizar</button>';
                        } else {
                            acciones += '<button class="btn btn-sm btn-success btnContabilizarMov" data-id="'+mov.id+'">Contabilizar</button>';
                        }
                    } else {
                        acciones = motivoAnulacion;
                    }
                    var badge = '';
                    if (mov.estado === 'previsualizacion') badge = '<span class="badge badge-warning">Solicitud</span>';
                    else if(mov.tipo_transferencia === 'abono') badge = '<span class="badge badge-success">Abono</span>';
                    else if(mov.tipo_transferencia === 'cargo') badge = '<span class="badge badge-danger">Cargo</span>';
                    else badge = '<span class="badge badge-secondary">'+(mov.tipo_transferencia||'')+'</span>';
                    ...
                    $tbody.append(
                        ...
                        '<td class="texto-suave">'+(mov.ejecutado_por || '-')+'</td>'+
                        '<td>'+(mov.referencia1 || '')+'</td>'+
                        '<td>'+(mov.referencia2 || '')+'</td>'+
