    ...
    <div class="main-content">
        <div class="container-fluid">
            <style>
                ...
                .teso-monto-split .split-line {
                    display: grid;
                    grid-template-columns: 44px 1fr;
                    align-items: center;
                    gap: 6px;
                }
                .teso-monto-split .split-label {
                    font-size: .78rem;
                    font-weight: 700;
                    color: #244d74;
                    text-align: right;
                }
                .teso-monto-split .split-total {
                    font-size: .78rem;
                    color: #5b7088;
                    margin-top: 1px;
                }
                ...
                .teso-serie-chip strong {
                    color: #0f365d;
                    font-weight: 700;
                }
                .teso-serie-chip.neutral {
                    color: #5b6f86;
                }
                .teso-serie-chip.money-usd {
                    border-color: #cde2ff;
                    background: #edf5ff;
                    color: #123f70;
                }
                .teso-serie-chip.money-nio {
                    border-color: #d4eddc;
                    background: #edf9f1;
                    color: #1f5d3f;
                }
            ...
            <?php
                ...
                $extraerMontosPago = function ($p) use ($normalizarMoneda) {
                    $usd = isset($p->monto_usd) ? floatval($p->monto_usd) : 0;
                    $nio = isset($p->monto_nio) ? floatval($p->monto_nio) : 0;
                    $montoBase = isset($p->monto) ? floatval($p->monto) : 0;

                    if ($usd > 0 || $nio > 0) {
                        return array(
                            'usd' => round(max(0, $usd), 2),
                            'nio' => round(max(0, $nio), 2),
                            'base' => $montoBase,
                        );
                    }

                    $monedaTmp = $normalizarMoneda(isset($p->moneda) ? $p->moneda : 'USD');
                    return array(
                        'usd' => $monedaTmp === 'NIO' ? 0 : round(max(0, $montoBase), 2),
                        'nio' => $monedaTmp === 'NIO' ? round(max(0, $montoBase), 2) : 0,
                        'base' => $montoBase,
                    );
                };
                    ...
                    <div class="d-none d-md-block">
                        <?php foreach ($pendientesPorSerie as $serie => $rowsSerie): ?>
                                <?php
                                    ...
                                    $serieResumen = array(
                                        'recibos' => count($rowsSerie),
                                        'usd_count' => 0,
                                        'usd_monto' => 0,
                                        'nio_count' => 0,
                                        'nio_monto' => 0,
                                        'metodos' => array(
                                            'efectivo' => array('count' => 0, 'usd' => 0, 'nio' => 0),
                                            'cheque' => array('count' => 0, 'usd' => 0, 'nio' => 0),
                                            'transferencia' => array('count' => 0, 'usd' => 0, 'nio' => 0),
                                            'otros' => array('count' => 0, 'usd' => 0, 'nio' => 0),
                                        ),
                                    );
                                    $serieRefNums = array();
                                    $serieRefLen = 0;
                                    if (!empty($rowsSerie)) {
                                        foreach ($rowsSerie as $tmpSerieRow) {
                                            ...
                                            $montoTmpSerie = isset($tmpSerieRow->monto_recibido) && $tmpSerieRow->monto_recibido !== null
                                                ? floatval($tmpSerieRow->monto_recibido)
                                                : (isset($tmpSerieRow->monto) ? floatval($tmpSerieRow->monto) : 0);
                                            $montosSerie = $extraerMontosPago($tmpSerieRow);
                                            $usdSerie = $montosSerie['usd'];
                                            $nioSerie = $montosSerie['nio'];

                                            if ($usdSerie > 0) {
                                                $serieResumen['usd_count']++;
                                                $serieResumen['usd_monto'] += $usdSerie;
                                            }
                                            if ($nioSerie > 0) {
                                                $serieResumen['nio_count']++;
                                                $serieResumen['nio_monto'] += $nioSerie;
                                            }

                                            $medioTmpSerie = isset($tmpSerieRow->medio_pago) ? strtolower(trim((string)$tmpSerieRow->medio_pago)) : '';
                                            $metodoKey = 'otros';
                                            if (strpos($medioTmpSerie, 'transfer') !== false) {
                                                $metodoKey = 'transferencia';
                                            } elseif (strpos($medioTmpSerie, 'cheq') !== false) {
                                                $metodoKey = 'cheque';
                                            } elseif (strpos($medioTmpSerie, 'efect') !== false) {
                                                $metodoKey = 'efectivo';
                                            }
                                            $serieResumen['metodos'][$metodoKey]['count']++;
                                            $serieResumen['metodos'][$metodoKey]['usd'] += $usdSerie;
                                            $serieResumen['metodos'][$metodoKey]['nio'] += $nioSerie;

                                            $refTmpSerie = isset($tmpSerieRow->documento_numero) ? trim((string)$tmpSerieRow->documento_numero) : '';
                                            if ($refTmpSerie !== '' && preg_match('/^' . preg_quote((string)$serie, '/') . '(\d+)$/i', $refTmpSerie, $mRefSerie)) {
                                                $serieRefNums[] = intval($mRefSerie[1]);
                                                $serieRefLen = max($serieRefLen, strlen($mRefSerie[1]));
                                            }
                                        }
                                    }
                                    ...
                                    $serieRefDesde = 'N/D';
                                    $serieRefHasta = 'N/D';
                                    if (!empty($serieRefNums)) {
                                        sort($serieRefNums, SORT_NUMERIC);
                                        $lenPad = max(10, intval($serieRefLen));
                                        $serieRefDesde = $serie . str_pad((string)$serieRefNums[0], $lenPad, '0', STR_PAD_LEFT);
                                        $serieRefHasta = $serie . str_pad((string)$serieRefNums[count($serieRefNums) - 1], $lenPad, '0', STR_PAD_LEFT);
                                    }
                                ?>
                                <div class="table-responsive mb-3">
                                    ...
                                    <div class="teso-serie-preview">
                                        <span class="teso-serie-chip"><strong>Recibos:</strong> <?php echo number_format($serieResumen['recibos'], 0); ?></span>
                                        <span class="teso-serie-chip neutral"><strong>Rango:</strong> <?php echo html_escape($serieRefDesde); ?> a <?php echo html_escape($serieRefHasta); ?></span>
                                        <span class="teso-serie-chip money-usd"><strong>USD:</strong> $<?php echo number_format($serieResumen['usd_monto'], 2); ?> (<?php echo number_format($serieResumen['usd_count'], 0); ?>)</span>
                                        <span class="teso-serie-chip money-nio"><strong>NIO:</strong> C$<?php echo number_format($serieResumen['nio_monto'], 2); ?> (<?php echo number_format($serieResumen['nio_count'], 0); ?>)</span>
                                        <span class="teso-serie-chip"><strong>Efectivo:</strong> <?php echo number_format($serieResumen['metodos']['efectivo']['count'], 0); ?> | $<?php echo number_format($serieResumen['metodos']['efectivo']['usd'], 2); ?> | C$<?php echo number_format($serieResumen['metodos']['efectivo']['nio'], 2); ?></span>
                                        <span class="teso-serie-chip"><strong>Cheque:</strong> <?php echo number_format($serieResumen['metodos']['cheque']['count'], 0); ?> | $<?php echo number_format($serieResumen['metodos']['cheque']['usd'], 2); ?> | C$<?php echo number_format($serieResumen['metodos']['cheque']['nio'], 2); ?></span>
                                        <span class="teso-serie-chip"><strong>Transferencia:</strong> <?php echo number_format($serieResumen['metodos']['transferencia']['count'], 0); ?> | $<?php echo number_format($serieResumen['metodos']['transferencia']['usd'], 2); ?> | C$<?php echo number_format($serieResumen['metodos']['transferencia']['nio'], 2); ?></span>
                                        <?php if ($serieResumen['metodos']['otros']['count'] > 0): ?>
                                            <span class="teso-serie-chip neutral"><strong>Otros:</strong> <?php echo number_format($serieResumen['metodos']['otros']['count'], 0); ?> | $<?php echo number_format($serieResumen['metodos']['otros']['usd'], 2); ?> | C$<?php echo number_format($serieResumen['metodos']['otros']['nio'], 2); ?></span>
                                        <?php endif; ?>
                                    </div>
                                        ...
                                        <tbody>
                                            ...
                                            <?php foreach ($rowsSerie as $i => $row): ?>
                                                <?php
                                                    ...
                                                    $montosRow = $extraerMontosPago($row);
                                                    $usdRow = $montosRow['usd'];
                                                    $nioRow = $montosRow['nio'];
                                                    $isMixtoRow = ($usdRow > 0 && $nioRow > 0);
                                                    if ($isMixtoRow) {
                                                        $monedaRow = 'MIXTO';
                                                    } elseif ($nioRow > 0) {
                                                        $monedaRow = 'NIO';
                                                    ...
                                                    $simboloRow = $simboloMoneda($monedaRow === 'MIXTO' ? 'USD' : $monedaRow);
                                                    ...
                                                    $detalleMontoRow = 'Desglose monetario: USD $' . number_format($usdRow, 2) . ' | NIO C$' . number_format($nioRow, 2);
                                                ?>
                                                        <tr class="<?php echo $refSospechosa ? 'teso-row-risk' : ''; ?> js-pago-row" data-row-id="<?php echo intval(isset($row->id) ? $row->id : 0); ?>" data-row-monto="<?php echo number_format($montoSistema, 2, '.', ''); ?>" data-row-monto-usd="<?php echo number_format($usdRow, 2, '.', ''); ?>" data-row-monto-nio="<?php echo number_format($nioRow, 2, '.', ''); ?>" data-row-mixed="<?php echo $isMixtoRow ? '1' : '0'; ?>" data-row-moneda="<?php echo html_escape($monedaRow); ?>" data-serie="<?php echo html_escape($serie); ?>" data-ref-ok="<?php echo $refValida ? '1' : '0'; ?>" data-ready="0">
                                                    <td><?php echo $i + 1; ?></td>
                                                    ...
                                                    <td>
                                                        <?php echo $simboloRow; ?><?php echo number_format($montoSistema, 2); ?>
                                                        <div class="teso-monto-distrib"><?php echo html_escape($detalleMontoRow); ?></div>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td style="min-width:190px;">
                                                        <?php if ($isMixtoRow): ?>
                                                            <div class="teso-monto-split">
                                                                <div class="split-line">
                                                                    <span class="split-label">USD</span>
                                                                    <input type="number" step="0.01" min="0" class="form-control form-control-sm input-monto-usd-recibido" value="<?php echo number_format($usdRow, 2, '.', ''); ?>" placeholder="0.00">
                                                                </div>
                                                                <div class="split-line">
                                                                    <span class="split-label">NIO</span>
                                                                    <input type="number" step="0.01" min="0" class="form-control form-control-sm input-monto-nio-recibido" value="<?php echo number_format($nioRow, 2, '.', ''); ?>" placeholder="0.00">
                                                                </div>
                                                                <div class="split-total">Total equiv. USD: <strong class="js-total-usd-row"><?php echo number_format($montoSistema, 2, '.', ''); ?></strong></div>
                                                            </div>
                                                        <?php else: ?>
                        ...
                        <?php else: ?>
                            <?php foreach ($pendientes as $i => $row): ?>
                                <?php
                                    ...
                                    $montosMobile = $extraerMontosPago($row);
                                    $usdMobile = $montosMobile['usd'];
                                    $nioMobile = $montosMobile['nio'];
                                    $isMixtoMobile = ($usdMobile > 0 && $nioMobile > 0);
                                    if ($isMixtoMobile) {
                                        $monedaRowMobile = 'MIXTO';
                                    } elseif ($nioMobile > 0) {
                                        $monedaRowMobile = 'NIO';
                                    } else {
                                        $monedaRowMobile = 'USD';
                                    }
                                    $simboloRowMobile = $simboloMoneda($monedaRowMobile === 'MIXTO' ? 'USD' : $monedaRowMobile);
                                    ...
                                    $detalleMontoMobile = 'Desglose monetario: USD $' . number_format($usdMobile, 2) . ' | NIO C$' . number_format($nioMobile, 2);
                                ?>
                                    <div class="teso-mobile-card <?php echo $refSospechosa ? 'teso-row-risk' : ''; ?> js-pago-row" data-row-id="<?php echo intval(isset($row->id) ? $row->id : 0); ?>" data-row-monto="<?php echo number_format($montoSistemaMobile, 2, '.', ''); ?>" data-row-monto-usd="<?php echo number_format($usdMobile, 2, '.', ''); ?>" data-row-monto-nio="<?php echo number_format($nioMobile, 2, '.', ''); ?>" data-row-mixed="<?php echo $isMixtoMobile ? '1' : '0'; ?>" data-row-moneda="<?php echo html_escape($monedaRowMobile); ?>" data-serie="<?php echo html_escape($serieMobile); ?>" data-ref-ok="<?php echo $refValidaMobile ? '1' : '0'; ?>" data-ready="0">
                                    <div class="head">
                                        <div>
                                    ...
                                    <div class="line"><strong>Detalle:</strong><?php echo html_escape($detalleMontoMobile); ?></div>
                                    <div class="line">
                                    ...
                                    <div class="line"><strong>Monto a Aplicar:</strong>
                                        <?php if ($isMixtoMobile): ?>
                                            <div class="teso-monto-split mt-1">
                                                <div class="split-line">
                                                    <span class="split-label">USD</span>
                                                    <input type="number" step="0.01" min="0" class="form-control form-control-sm input-monto-usd-recibido" placeholder="0.00" value="<?php echo number_format($usdMobile, 2, '.', ''); ?>">
                                                </div>
                                                <div class="split-line">
                                                    <span class="split-label">NIO</span>
                                                    <input type="number" step="0.01" min="0" class="form-control form-control-sm input-monto-nio-recibido" placeholder="0.00" value="<?php echo number_format($nioMobile, 2, '.', ''); ?>">
                                                </div>
                                                <div class="split-total">Total equiv. USD: <strong class="js-total-usd-row"><?php echo number_format($montoSistemaMobile, 2, '.', ''); ?></strong></div>
                                            </div>
                                        <?php else: ?>
                                            <input type="number" step="0.01" min="0" class="form-control form-control-sm input-monto-recibido" placeholder="0.00" value="<?php echo ($montoRecMobile !== null) ? number_format($montoRecMobile, 2, '.', '') : ''; ?>">
                                        <?php endif; ?>
                                    </div>
...
<script>
    (function () {
        ...
        function getRowSplitAmounts($row, tasaVenta) {
            var isMixed = (($row.attr('data-row-mixed') || '0') === '1');
            var usd = 0;
            var nio = 0;
            var totalUsd = 0;

            if (isMixed) {
                usd = parseAmount($row.find('.input-monto-usd-recibido').val());
                nio = parseAmount($row.find('.input-monto-nio-recibido').val());
                if (nio > 0 && (!tasaVenta || tasaVenta <= 0)) {
                    return { valid: false, usd: usd, nio: nio, totalUsd: 0 };
                }
                totalUsd = usd + (nio > 0 ? (nio / tasaVenta) : 0);
                return { valid: (totalUsd > 0), usd: usd, nio: nio, totalUsd: totalUsd };
            }

            totalUsd = parseAmount($row.find('.input-monto-recibido').val());
            var moneda = (($row.attr('data-row-moneda') || 'USD') + '').toUpperCase();
            if (moneda === 'NIO') {
                nio = totalUsd;
                usd = 0;
            } else {
        ...
        function refreshRowSplitTotal($row) {
            if (!$row || !$row.length) return;
            var isMixed = (($row.attr('data-row-mixed') || '0') === '1');
            if (!isMixed) return;
            var tasas = getTasaInputs();
            var split = getRowSplitAmounts($row, tasas.venta);
            var txt = split.valid ? split.totalUsd.toFixed(2) : '0.00';
            $row.find('.js-total-usd-row').text(txt);
        }
        ...
        function evaluateRow($row) {
            ...
            var tasas = getTasaInputs();
            var split = getRowSplitAmounts($row, tasas.venta);
            var montoRecibido = split.totalUsd;
            ...
            var montoSistema = parseAmount($row.attr('data-row-monto') || '0');
            var cuadra = (Math.abs(montoSistema - montoRecibido) < 0.01);
            if (!refOk) {
                setRowReady($row, false);
            } else if (!split.valid || !montoRecibido || fechaRecepcion === '') {
                setRowReady($row, false);
            } else if (!cuadra) {
                setRowReady($row, false);
            } else if (!revisado) {
            ...
            return (refOk && revisado && split.valid && montoRecibido > 0 && fechaRecepcion !== '' && cuadra);
        }
        ...
        $('#btnGuardarRecepcion').on('click', function () {
            ...
            $('.js-pago-row[data-row-id]').each(function () {
                ...
                var split = getRowSplitAmounts($row, tasas.venta);
                var montoRec = split.totalUsd || 0;
                var fechaRec = $row.find('.input-fecha-recepcion').val() || '';
                if (montoRec > 0 && fechaRec && split.valid) {
                    mapItems[id] = {
                        id: id,
                        monto_recibido: montoRec,
                        monto_usd_recibido: split.usd || 0,
                        monto_nio_recibido: split.nio || 0,
            ...
            $('.js-pago-row[data-ready="1"]').each(function () {
                ...
                var splitCheck = getRowSplitAmounts($(this), tasas.venta);
                var montoNio = splitCheck.nio || 0;
                if (m === 'NIO' || montoNio > 0) {
                    requiereVenta = true;
                }
            });
        ...
        $(document).on('input change', '.input-monto-recibido, .input-monto-usd-recibido, .input-monto-nio-recibido, .input-fecha-recepcion', function () {
            var $row = $(this).closest('.js-pago-row');
            refreshRowSplitTotal($row);
            setRowReady($row, false);
            evaluateRow($row);
        ...
        $(document).on('input', '#tc_venta', function () {
            ...
            if (isNaN(val) || val <= 0) {
                ...
                $('.js-pago-row').each(function () { refreshRowSplitTotal($(this)); });
                return;
            }
