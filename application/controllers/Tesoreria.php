...
class Tesoreria extends CI_Controller {
    ...
    public function save_concepto_ajax() {
        ...
        public function get_movimientos_ajax() {
            ...
            foreach($movs as &$m){
                ...
                $m['ejecutado_por'] = isset($m['creado_por']) && trim((string)$m['creado_por']) !== '' ? trim((string)$m['creado_por']) : '-';
    ...
    public function save_cierre_arqueo_ajax()
    {
        ...
        $estadoDeposito = strtolower(trim((string)$this->input->post('estado_deposito')));
        if (!in_array($estadoDeposito, array('pendiente', 'depositado'))) {
            $estadoDeposito = 'pendiente';
        }

        $idbanco = intval($this->input->post('idbanco'));
        $montoDeposito = floatval($this->input->post('monto_depositado_total'));
        ...
        if ($estadoDeposito === 'depositado') {
            if ($idbanco <= 0) {
                echo json_encode(array('status' => false, 'message' => 'Debe seleccionar banco para registrar el depósito'));
                return;
            }
            if ($montoDeposito <= 0) {
                echo json_encode(array('status' => false, 'message' => 'Debe indicar monto depositado total'));
                return;
            }
            if ($referenciaMinuta === '' || $fechaDeposito === '') {
                echo json_encode(array('status' => false, 'message' => 'Debe indicar referencia de minuta y fecha de depósito'));
                return;
            }
        }

        $existing = $this->_get_cierre_arqueo($cierre_id);
        if ($existing && isset($existing['estado_deposito']) && $existing['estado_deposito'] === 'depositado' && $estadoDeposito === 'depositado') {
            echo json_encode(array('status' => false, 'message' => 'Este cierre ya fue depositado anteriormente'));
            return;
        }
        ...
        try {
            $dataArqueo = array(
                ...
                'idbanco' => $idbanco > 0 ? $idbanco : null,
                'estado_deposito' => $estadoDeposito,
                'monto_depositado_total' => $estadoDeposito === 'depositado' ? $montoDeposito : null,
                'referencia_minuta' => $estadoDeposito === 'depositado' ? $referenciaMinuta : null,
                'fecha_deposito' => $estadoDeposito === 'depositado' ? $fechaDeposito : null,
                'idusuario' => $usuario_id,
            ...
            if ($estadoDeposito === 'depositado') {
                ...
                $mov = array(
                    ...
                    'concepto' => 'Deposito de arqueo cierre #' . intval($cierre->consecutivo),
                    'forma_pago' => 'DEPOSITO',
                    'fecha_registro' => $fechaDeposito,
                    'fecha_aplicacion' => $fechaDeposito,
                    'beneficiario' => 'TESORERIA',
                    'referencia1' => $referenciaMinuta,
                    'referencia2' => 'cierre_id=' . $cierre_id,
                    'monto_total' => $montoDeposito,
                    'iva_total' => 0,
                    'descripcion' => 'Deposito de arqueo - cierre de caja #' . intval($cierre->consecutivo),
                    'cuenta_id' => $idbanco,
                ...
                $this->db->set('total_abonos', 'COALESCE(total_abonos,0)+' . $this->db->escape($montoDeposito), false);
                $this->db->set('saldo_actual', 'COALESCE(saldo_actual,0)+' . $this->db->escape($montoDeposito), false);
                $this->db->where('id', $idbanco)->update('teso_accounts');

                $this->db->where('id', $idarqueo)->update('tb_cierre_arqueos', array('deposito_movimiento_id' => $movId));
            }
            ...
            echo json_encode(array(
                'status' => true,
                'message' => $estadoDeposito === 'depositado'
                    ? 'Arqueo guardado y depósito aplicado al banco correctamente'
                    : 'Arqueo guardado. Depósito pendiente de aplicación a banco',
    ...
    public function guardar_recepcion_pagos_ajax()
    {
        ...
        foreach ($items as $it) {
            ...
            $result = $this->_aplicar_pago_provisional($id, $monto_recibido, $fecha_recepcion, $tasa_venta, $tasa_compra);
    ...
    private function _aplicar_pago_provisional($pago_id, $monto_recibido_post, $fecha_recepcion_post, $tasa_venta_input = 0, $tasa_compra_input = 0)
    {
        $this->ensure_teso_pagos_recepcion_columns();
