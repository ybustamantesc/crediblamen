! function(e, s, i) {
    ...
    i(s).ready(function() {
        ...
        try {
            var _rs_el = document.querySelector('.right-sidebar');
            if (_rs_el) new PerfectScrollbar(_rs_el, { wheelSpeed: 10, wheelPropagation: true, minScrollbarLength: 5 });
        } catch(err) { console.error('PerfectScrollbar init right-sidebar error', err); }
        try {
            var _msgs_el = document.querySelector('.messages');
            if (_msgs_el) new PerfectScrollbar(_msgs_el, { wheelSpeed: 10, wheelPropagation: true, minScrollbarLength: 5 });
        } catch(err) { console.error('PerfectScrollbar init messages error', err); }
        $(".right-sidebar-toggle").on("click",function(e) {
            ...
            return false;
                ...
                document.addEventListener('click', function(event) {
                        ...
                        var isInsideContainer = ($rightSidebar && $rightSidebar.contains(event.target)) || ($chatPanel && $chatPanel.contains(event.target));
                        if( !isInsideContainer ) {
                            ...
                            if ($chatPanel) $chatPanel.hidden = 'hidden';
                        }
                });
